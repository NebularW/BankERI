    ...
    for dir_path, dir_names, filenames in os.walk(root_dir):
        for filename in filenames:
            if filename.endswith('.docx') or filename.endswith('.doc'):
                ...
                rule = Rule(index, name, category, path, None)
                rules.append(rule)
                index += 1
    return rules
...
def get_external_rule(root_dir, file_name):
    return Rule(0, file_name, "", os.path.join(root_dir, file_name + ".txt"), None)
...
def get_chinese_text(path):
    ...
    chinese_text = process_chinese_text(text)
    return chinese_text
...
class Rule:
    def __init__(self, rule_id, name, category, path, text):
        self.rule_id = rule_id
        self.name = name
        self.category = category
        self.path = path
        if text is None:
            self.ori = get_original_text(path)
            self.text = get_chinese_text(path)
            self.words = cut.cut_words(self.text)
        else:
            self.ori = text
            self.text = process_chinese_text(text)
            self.words = cut.cut_words(self.text)
        self.sim = 0.0
